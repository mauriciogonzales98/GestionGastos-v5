...
| Barrera del aislamiento (backend) | `./backend/verificar-aislamiento.sh` — comprueba que la barrera que exige que ninguna cuenta vea ni toque los datos de otra sabe fallar. Desarma el aislamiento de las **cinco** formas posibles —la consulta deja de acotar por cuenta; una lectura nace fuera del canal único; una lectura sin acotar nace dentro del archivo que la barrera exime por escribir; una consulta del canal no acota y encima no devuelve movimientos, así que la barrera ni la enumeraba; el alta le pone dueño ajeno a lo que escribe— y exige el rojo en cada una. Tarda ~5 min: son siete pasos y cada uno recompila. Sin esto, un test de aislamiento roto se ve igual que uno que funciona: devuelve verde y sigue devolviéndolo el día que deja de verificar nada |
