...
| Barrera del aislamiento (backend) | `./backend/verificar-aislamiento.sh` — comprueba que la barrera que exige que ninguna cuenta vea ni toque los datos de otra sabe fallar. Desarma el aislamiento de las **ocho** formas posibles —la consulta deja de acotar por cuenta; una lectura nace fuera del canal único; una lectura sin acotar nace dentro del archivo que la barrera exime por escribir; una consulta del canal no acota y encima no devuelve movimientos, así que la barrera ni la enumeraba; una consulta del canal ejecuta adentro y devuelve el resultado ya calculado, así que tampoco aparecía; una consulta del canal de **categorías** deja de acotar por ámbito, que es una tabla que hasta la feature 007 no tenía nada que aislar; una lectura de categorías nace **fuera** de su canal, que era la mitad que le faltaba a esa tabla —la deuda D7-05, saldada en la rama `017`, y su enunciado estaba equivocado: daba por hecho que no existía ninguna, y había dos—; el alta le pone dueño ajeno a lo que escribe— y exige el rojo en cada una. Tarda ~4 min **medidos** con los diez pasos, y cada uno recompila. El número anterior —~7 min— nunca se había cronometrado: se midió al agregar el décimo paso y resultó ser menos que el que figuraba con nueve, así que no lo subas por estimación, medilo. Sin esto, un test de aislamiento roto se ve igual que uno que funciona: devuelve verde y sigue devolviéndolo el día que deja de verificar nada |
| Barrera de la nota (backend) | `./backend/verificar-nota.sh` — comprueba que la barrera que mantiene a la nota **descriptiva y no clasificatoria** (RF-33) sabe fallar: le cuela un acotado por nota al listado, un orden por nota al listado y un agrupamiento por nota al resumen, y exige el rojo en cada uno. Existe desde la feature 012 y **no figuraba en esta tabla ni en `ci.yml`**; las dos cosas se arreglaron en la 013, al agregar la octava. La del orden es la que prueba el punto del script mejor que ninguna: hasta el PR #29 la barrera cubría dos tercios de su requisito afirmando cubrirlo entero, porque el recorte que aísla el `WHERE` corta justo en el `ORDER BY`. Recompila, así que va después de los tests |
| Barrera del ámbito de categoría (backend) | `./backend/verificar-ambito-de-categoria.sh` — la octava. Comprueba que la barrera que exige que **la base misma** rechace un movimiento clasificado con la categoría de otra cuenta sabe ponerse en rojo: baja la clave foránea compuesta con `ALTER TABLE`, exige el rojo, la restaura y exige el verde. Es la deuda D7-07, saldada por la feature 013 — hasta entonces la regla la sostenían sólo dos comprobaciones de la aplicación, así que valía para lo que pasa por la API y no para un script de mantenimiento, una importación o un arreglo a mano en la base. **Se desarma con SQL y no tocando código**, porque la restricción no vive en una consulta de C#: la puso una migración y vive en el catálogo de MySQL. Necesita el cliente `mysql`, igual que la de monedas. No recompila y tarda segundos |
