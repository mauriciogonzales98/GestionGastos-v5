...
| Barrera del aislamiento (backend) | `./backend/verificar-aislamiento.sh` — comprueba que la barrera que exige que ninguna cuenta vea ni toque los datos de otra sabe fallar. Desarma el aislamiento de las **tres** formas posibles —la consulta deja de acotar por cuenta, una lectura nace fuera del canal único, el alta le pone dueño ajeno a lo que escribe— y exige el rojo en cada una. Sin esto, un test de aislamiento roto se ve igual que uno que funciona: devuelve verde y sigue devolviéndolo el día que deja de verificar nada |
