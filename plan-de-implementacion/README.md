...
| `014` | **D12-08** — la doble representación de "sin nota" en el almacenamiento, la única deuda que la feature 012 **creó** y la única que nació aceptada a sabiendas en sus *Clarifications* | Saldada, y el hallazgo fue **dónde** faltaba: normalizar al escribir ya se hacía —`ValidacionDelMovimiento.NotaNormalizada` convierte la cadena vacía y los espacios en ausencia de valor desde la 012—, así que lo que no existía no era la normalización sino algo que la **garantizara**. El camino por el que el segundo estado podía entrar es justamente el que no pasa por la aplicación, y sólo una restricción de esquema lo alcanza: `ck_movimiento_nota_sin_cadena_vacia` (`FR-014`), con el mismo patrón con el que la 012 cerró D11-02 sobre `moneda.codigo`. La migración **normaliza antes de restringir**, porque MySQL se niega a agregar un `CHECK` que las filas existentes ya incumplen. Y le quitó la mitad que dejó de tener premisa a un test —el `UPDATE` con `''` que forzaba ahora lo rechaza la base—: esa mitad se mudó a `NotaSinCadenaVaciaEsquemaTests`, donde es una restricción verificada en vez de una igualdad confiada a la disciplina. El detalle está en el cierre de [`specs/012-nota-del-movimiento/spec.md`](../specs/012-nota-del-movimiento/spec.md) |
| `016` | **D6-06** — el resumen y el listado podían mostrar cifras que se contradecían en la misma pantalla, abierta desde la feature 006 | El contrato de `GET /api/resumen` ya había dejado escritos **dos** caminos —*"o el filtro tapa las dos vistas a la vez, o el resumen se titula de forma que se vea que habla de todo el período"*— y se tomó el segundo, que era el que el PRD quería: el resumen es del período completo por diseño. Lo que faltaba era **terminarlo**: ya declaraba su período en pantalla desde la 010, pero nada decía sobre la categoría, que es justamente el acotado que no sabe hacer. **Lo que decide si el aviso dice la verdad es qué mira**: lo aplicado contra lo que el resumen cubre, y no si alguien tocó "Aplicar" — la barra viene sembrada con el período del resumen, así que aplicar sin cambiar nada pide exactamente el mes que el resumen ya cuenta, y ahí un aviso mentiría. El backend no se tocó. El detalle está en el cierre de [`specs/006-resumen-del-mes/spec.md`](../specs/006-resumen-del-mes/spec.md) |
| `017` | **D7-05** — la barrera del canal de categorías vigilaba el canal por dentro y no quién lo esquiva, abierta desde la feature 007 | Saldada, y **su enunciado estaba equivocado en el hecho que lo sostenía**: decía que no había ninguna lectura de categorías fuera del canal y que lo único que faltaba era proteger lo que alguien escribiera después. Había **dos**, las dos en `MovimientosEndpoints.cs` y las dos anteriores a la deuda. No había ningún dato expuesto —las dos llevaban el acotado por ámbito escrito a mano y bien escrito—; lo que no había era nada que las obligara a seguir estándolo. Se mudaron al canal con `CategoriasConsulta.DelAmbitoPorId`, que es la salida que la barrera de movimientos predica desde FEAT-001b: *se agrega el método al canal, no la excepción a la barrera*. **El rojo no fue ceremonial**: el escaneo se escribió primero y falló nombrando a las dos lecturas de verdad. `verificar-aislamiento.sh` pasó de nueve pasos a diez. La revisión del PR encontró además que el escaneo entraba a `obj/` y `bin/`, y que la duración que la tabla de *Stack* declaraba nunca se había medido. El detalle está en el cierre de [`specs/007-categorias-propias/spec.md`](../specs/007-categorias-propias/spec.md) |
| `022` | **D7-07** — ningún `CHECK` de esquema impedía que un movimiento apuntara a una categoría fuera del ámbito de su dueño, abierta al saldar D7-05 y anotada en la rama `020` | Saldada, y **no por ninguno de los dos caminos que la fila proponía**: los dos se midieron y los dos fallaron. La clave foránea compuesta rechazaba las diez predefinidas con `ERROR 1452` —una fila padre con `NULL` en la clave referenciada no puede ser referenciada por nadie— y el trigger no se puede crear con las credenciales de la aplicación (`ERROR 1419`), aunque en CI, que conecta como `root`, habría salido verde y habría explotado recién al migrar en local. **El problema no era la restricción sino la forma de los datos**: mientras "predefinida" significara `usuario_id IS NULL`, ninguna restricción declarativa podía decir "el dueño coincide **o la categoría no es de nadie**". Por eso la feature 013 sacó el `NULL` del modelo: las diez categorías iniciales dejaron de ser filas compartidas y pasaron a entregarse, copiadas, a cada cuenta que se registra. Con toda categoría con dueño, la regla se volvió la misma clave foránea que acababa de fallar, funcionando. De regalo desapareció la categoría de solo lectura, y con ella el `403` y el campo `esPropia` del contrato. Trajo la **octava** barrera, `verificar-ambito-de-categoria.sh`, y destapó dos cosas que no eran de la feature: `verificar-nota.sh` no corría en CI desde la 012, y el conteo de "cinco tests que fijan identificadores de categoría" de research contaba una sola de las dos formas de escribirlos — eran más de cuarenta, en 16 archivos. El detalle está en [`specs/013-predefinidas-por-cuenta/spec.md`](../specs/013-predefinidas-por-cuenta/spec.md) |
