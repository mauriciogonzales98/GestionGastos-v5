...
| D8-07 | **El hash del ensamblado no detecta una recompilación**, y por eso `verificar-monedas.sh` mide además su fecha de modificación | .NET compila de forma determinista: recompilar el mismo fuente da un binario byte a byte idéntico. No es un defecto a arreglar, es una propiedad del compilador que hay que conocer — y quedó anotada porque el reflejo de "comparo hashes" es exactamente el que falla en silencio. Se descubrió corriendo el desarme, no leyendo el script | Nadie. Es documentación de una trampa, no trabajo pendiente |
| D8-08 | **Los tests de esta feature comparten el catálogo con la barrera que los corre**, y eso los acopla: un caso que afirme algo sobre el tamaño del catálogo se rompe cuando `verificar-monedas.sh` le agrega su moneda | Ya pasó una vez, con el `2` escrito a mano de AC-09, y lo encontró el quickstart y no la suite. Hoy está resuelto comparando contra el catálogo real, pero el acoplamiento sigue ahí para el próximo caso que alguien agregue | El ticket que agregue el siguiente test de monedas. La regla es corta: **nada de números fijos sobre el tamaño del catálogo** |
| D8-05 | **SALDADA por la feature 011.** **El formato regional del monto por moneda** —separadores, posición del símbolo—, y la columna `decimales` que ya existe y nadie usa | Es maquetación | Ticket 6 (Maquetación y accesibilidad) |
