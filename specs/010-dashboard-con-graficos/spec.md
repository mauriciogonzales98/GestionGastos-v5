...
|---|---|---|---|
| D10-01 | **SALDADA por la feature 011.** ~~La barra de filtros de categoría y de rango de fechas del listado, y la interfaz de eliminación de un movimiento~~ | Es **D9-01**, que sigue apuntando al mismo lugar. El rango de fechas que esta feature construye es el del **dashboard**: el del listado es otro control sobre otra pantalla, y arrastrarlo acá sería hacer la mitad de FEAT-001b de contrabando | Ticket 6 (Maquetación y accesibilidad) |
| D10-02 | **SALDADA por la feature 011.** ~~El formato regional del monto según la moneda y la columna `decimales`~~ | Es **D8-05** y **D9-05**, que siguen apuntando al mismo lugar | Ticket 6 (Maquetación y accesibilidad) |
...
| D10-10 | **IDENTIFICADO en la feature 011**: es `RendimientoLimiteTests…AC12`, el techo de 50 ms de `003:AC-12` — la sospecha era correcta. Falla en la corrida completa bajo carga y pasa aislado y en la de cobertura. Pasa a ser **D11-03**, que ya no es "averiguar cuál es" sino "decidir el techo". El texto original: ~~Un fallo único en la suite del backend, sin identificar.~~ En cinco corridas completas de `dotnet test backend/` durante el cierre, una dio `Failed: 1, Passed: 266`; las otras cuatro dieron 267 verdes, y los 12 tests de `Rendimiento/` corridos aparte dieron verde también | **La corrida roja fue la que siguió a `verificar-aislamiento.sh` y a la de cobertura**, o sea con la máquina bajo carga — que es exactamente la condición en la que **D9-08** describe a `RendimientoLimiteTests…AC12` fallando contra su techo de 50 ms. Pero **el nombre no se capturó** (la salida estaba recortada a las dos últimas líneas) y no se pudo reproducir en cuatro intentos, así que decir que fue ése sería una suposición, no un dato. El Principio IV exige arreglar o borrar un test intermitente; para eso primero hay que saber cuál es | El ticket que revise el techo de 50 ms de `003:AC-12` — el mismo de **D9-08**. Si vuelve a aparecer, correr con `--logger "trx"` y mirar el archivo: es la forma de no perder el nombre |
| D10-07 | **SALDADA por la feature 011**, que declara la paleta en `estilos/base.css` y la mide leyendo ese archivo. El texto original: ~~La paleta del dashboard son cuatro colores en `ui/contraste.ts`~~, y dos de ellos —el texto y el fondo— son los del navegador escritos a mano, no una elección | El proyecto no declara colores: `estilos/base.css` dice que son del ticket 6. Inventar acá una paleta sería hacer ese trabajo antes de tiempo y para que lo rehagan. Los cuatro valores están en un solo archivo y el test de contraste los mide, así que cuando llegue la paleta se ve enseguida si el par elegido no llega a 4,5:1 | Ticket 6 (Maquetación y accesibilidad) |
| D10-08 | **Ponerle color a las barras cuando exista la paleta** | D-04 dejó todas las barras del mismo relleno a propósito, porque las categorías no se codifican por color. Si el ticket 6 decide que sí lleven color, es cambiar un relleno en un solo lugar — pero ahí sí hará falta la paleta categórica accesible que hoy no se justifica | Ticket 6, si lo decide |
| D10-09 | **Los pasos de navegador del quickstart (6 a 11 y 13) no se ejecutaron a mano** | No hay navegador en el entorno donde se implementó. Lo que verifican está cubierto por los tests del frontend que cada paso cita, así que lo que queda sin comprobar no es el comportamiento sino cómo se ve | Quien tenga un navegador delante |
