...
| D7-07 | **Ningún `CHECK` de esquema impide que un movimiento apunte a una categoría fuera del ámbito de su dueño.** Hoy lo impiden dos comprobaciones de aplicación —las de `MovimientosEndpoints` en el alta y en la edición— con su test cruzado `Un_Movimiento_No_Puede_Apuntar_A_Una_Categoria_Ajena_FR021_SC009` | **Sale de la revisión del PR #35, al saldar D7-05, y no es una exposición**: las dos comprobaciones están puestas y verificadas. Lo que falta es que la invariante viva en la base y no sólo en el código, que es lo que la vuelve cierta también para el `INSERT` que no pasa por la aplicación. Importa además porque es **lo que sostiene** que las lecturas por propiedad de navegación —`m.Categoria!.Nombre` en `MovimientosConsulta`— sean seguras: ésas no pasan por el canal y ninguna barrera las ve. Es el mismo patrón que la 012 usó para `moneda.codigo` y la 014 para la nota: un `CHECK` más un test de esquema contra SQL directo | El próximo ticket que abra una migración. Ojo con la forma: la condición cruza dos tablas (`movimiento.categoria_id` contra `categoria.usuario_id`), y un `CHECK` de MySQL no puede consultar otra tabla — así que probablemente sea un trigger o una clave foránea compuesta, no un `CHECK`, y eso hay que diseñarlo antes de prometerlo |
