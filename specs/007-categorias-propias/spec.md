...
| ~~D7-07~~ | ~~**Ningún `CHECK` de esquema impide que un movimiento apunte a una categoría fuera del ámbito de su dueño.**~~ | **SALDADA** por la feature 013 (2026-09-13), y **no por ninguno de los dos caminos que esta fila proponía**: los dos se midieron y fallaron. Ver abajo | — |

#### Cómo se saldó D7-07

La fila proponía dos formas posibles —un trigger o una clave foránea compuesta— y pedía
explícitamente diseñarlas antes de prometerlas. Se diseñaron, se midieron contra la base real el
2026-09-12, y **las dos quedaron descartadas**:

| Camino que proponía la fila | Resultado medido |
|---|---|
...
| **Clave foránea compuesta** `(categoria_id, usuario_id) → categoria (id, usuario_id)` | Se montó y se probaron los tres casos. Rechaza la categoría ajena **y también rechaza las diez predefinidas** (`ERROR 1452`): una fila padre con `NULL` en la clave referenciada no puede ser referenciada por nadie. No rompía el ataque: rompía el uso normal |
| **Trigger** `BEFORE INSERT` / `BEFORE UPDATE` | Descartado con evidencia, y por el motivo peor: crearlo falla con `ERROR 1419 (SUPER privilege / binary logging)` aun con `ALL PRIVILEGES` sobre la base de tests, y sobre la base de desarrollo el usuario de la aplicación ni siquiera tiene el privilegio `TRIGGER`. Y `ci.yml` conecta **como `root`**: una migración con un trigger adentro se habría creado sin chistar en CI, habría salido verde, y habría explotado recién al migrar en local. Es exactamente el verde que miente contra el que están escritas las barreras del repositorio |

**La conclusión del spike fue que el problema no era la restricción sino la forma de los datos.**
Mientras "predefinida" significara `usuario_id IS NULL`, ninguna restricción declarativa podía
expresar la regla: la condición real era *"el dueño de la categoría es el mismo que el del
movimiento, **o la categoría no es de nadie**"*, y una clave foránea no sabe decir "o nula".

Por eso la feature 013 **eliminó el `NULL` del modelo**: las diez categorías iniciales dejaron de ser
filas compartidas y pasaron a entregarse, copiadas, a cada cuenta que se registra. Con toda categoría
con dueño, la regla se volvió la misma clave foránea compuesta que acababa de fallar —funcionando,
porque desapareció el `NULL` que la rompía—. La sostiene
`backend/verificar-ambito-de-categoria.sh`, la octava barrera del proyecto.

**El resultado final es el que la fila pedía** —la invariante en la base, y con ella la seguridad
*por construcción* de las lecturas por propiedad de navegación como `m.Categoria!.Nombre`— pero el
camino era otro, y saldarla costó una feature entera en vez del "próximo ticket que abra una
migración" que esta fila estimaba. Es la tercera vez que una deuda anotada de este repositorio
resulta tener la premisa o el costo mal medidos.
