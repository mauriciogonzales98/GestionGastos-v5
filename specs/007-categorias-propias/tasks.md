...
- [ ] T001 Correr la puerta del backend y la del frontend sobre la rama recién sacada de `main`
...
- [ ] T002 [TEST] Crear `backend/GestionGastos.Api.Tests/Integracion/UnicidadDeCategoriasTests.cs`
...
- [ ] T003 Agregar `Discriminador` a `backend/GestionGastos.Api/Dominio/Categoria.cs` y rehacer el
...
- [ ] T004 Generar la migración con `dotnet ef migrations add` y aplicarla. **Si la migración toca
      alguna de las diez filas sembradas, algo se salió del alcance** (D-10). T002 tiene que quedar
      en verde
- [ ] T005 [TEST] Agregar a `UnicidadDeCategoriasTests` el caso de SC-005: después de migrar, las
      diez predefinidas siguen con el mismo `id`, el mismo nombre y el mismo tipo, y todas con
      `discriminador = 0`
- [ ] T006 [VERIFY] Puerta del backend completa, con su salida

### La barrera del canal de categorías (D-03)

- [ ] T007 Crear `backend/GestionGastos.Api/Categorias/CategoriasConsulta.cs` **moviendo** ahí la
...
- [ ] T008 [TEST] Agregar temporalmente a `CategoriasConsulta.cs` una consulta **deliberadamente sin
...
- [ ] T009 Extender `backend/GestionGastos.Api.Tests/Integracion/BarreraDeAislamientoTests.cs` para
...
- [ ] T010 [ROJO] Volver a correr la barrera con la consulta sin acotar todavía puesta. **Ahora
...
- [ ] T011 [TEST] Agregar a `backend/verificar-aislamiento.sh` un **séptimo desarme**: una consulta
...
- [ ] T012 [ROJO] Correr `./backend/verificar-aislamiento.sh` entero y mostrar su salida. Los siete
      desarmes tienen que dar rojo cada uno y el verde final tiene que volver
- [ ] T013 [VERIFY] Puerta del backend completa, con su salida

### La barrera del desglose: la deuda D6-04 (D-05)

- [ ] T014 [TEST] Agregarle temporalmente `&& m.Categoria!.Activa` a
      ...
      Mostrar la salida
- [ ] T015 Crear `backend/GestionGastos.Api.Tests/Integracion/BarreraDelDesgloseTests.cs`: inspecciona
...
- [ ] T016 [ROJO] Correr la barrera con el filtro todavía puesto. **ROJO.** Mostrar la salida.
      Recién ahí **quitar** el filtro y comprobar el verde
- [ ] T017 [TEST] Crear `backend/verificar-desglose.sh` —la quinta barrera del proyecto— que le
...
- [ ] T018 [ROJO] Correr `./backend/verificar-desglose.sh` entero y mostrar su salida
- [ ] T019 [P] Agregar la barrera nueva a la tabla de *Stack* de `AGENTS.md`, con su comando y su
      motivo, junto a las otras cuatro

### La higiene de la suite (Principio IV)

- [ ] T020 Ampliar `LimpiarCuentasAsync` en `backend/GestionGastos.Api.Tests/Integracion/` para que
...
- [ ] T021 [VERIFY] Puerta del backend completa, con su salida
...
- [ ] T022 [TEST] [US1] Crear `backend/GestionGastos.Api.Tests/Contrato/ContratoCategoriasTests.cs`
...
- [ ] T023 [US1] Agregar `EsPropia` a `backend/GestionGastos.Api/Categorias/CategoriaDto.cs` y
...
- [ ] T025 [US1] Escribir en `CategoriasConsulta.cs` la consulta del ámbito de una cuenta
      ...
      la use
- [ ] T026 [TEST] [US1] AC-01 en `CategoriasPropiasTests`: crear una categoría propia de gasto la
      hace aparecer en el catálogo de esa cuenta, con `esPropia: true`, y **no** en el de otra
- [ ] T027 [TEST] [P] [US1] AC-07 y FR-007: se rechaza el nombre repetido contra una **propia** y
...
- [ ] T028 [TEST] [P] [US1] AC-10: nombre vacío, en blanco y de 51 caracteres se rechazan con la
      clave `nombre`; el de 50 se acepta
- [ ] T029 [TEST] [P] [US1] AC-08: dos cuentas distintas crean cada una la misma categoría y las dos
      se aceptan, y cada una ve sólo la suya
- [ ] T030 [TEST] [P] [US1] Sin sesión, `POST /api/categorias` responde `401` (RF-03)
- [ ] T031 [TEST] [P] [US1] Mismo nombre, **otro tipo** se acepta: la unicidad es por `(nombre, tipo)`
- [ ] T032 [US1] Crear `backend/GestionGastos.Api/Categorias/ValidacionDeLaCategoria.cs` con el
...
- [ ] T033 [US1] Implementar `POST /api/categorias` en `CategoriasEndpoints.cs`: `201` con
      `Location`, la fila nace con `Activa = true` y `Discriminador = 0`
- [ ] T034 [TEST] [US1] Agregar a `ContratoCategoriasTests` la petición del alta (`NuevaCategoria`) y
      su respuesta, en las dos direcciones
- [ ] T035 [P] [US1] Declarar `NuevaCategoria` en `frontend/src/api/tipos.ts` y `crearCategoria` en
      `frontend/src/api/cliente.ts`
- [ ] T036 [VERIFY] [US1] Puerta del backend y del frontend completas, con su salida
...
- [ ] T037 [TEST] [US2] AC-04 en `CategoriasPropiasTests`: renombrar una categoría propia con
...
- [ ] T038 [TEST] [P] [US2] Clarificación 1: el renombre valida la misma unicidad que el alta
      —contra propias y contra predefinidas—, y **no choca consigo misma**: renombrar "Gimnasio" a
      "Gimnasio" no es un error
- [ ] T039 [TEST] [P] [US2] AC-03: `PUT` sobre una categoría **predefinida** responde `403` y la deja
      con el mismo nombre y el mismo tipo. **No** es `404`: la persona la está viendo
      ([D-06](./research.md#d-06--qué-responde-cada-rechazo))
- [ ] T040 [TEST] [P] [US2] AC-11 y FR-013: `PUT` sobre una categoría propia de **otra** cuenta
      responde `404` **con el mismo cuerpo** que un identificador inexistente, y la deja sin cambios
- [ ] T041 [TEST] [P] [US2] AC-10 en el renombre: nombre vacío o de más de 50 se rechaza y la
      categoría queda como estaba
- [ ] T042 [US2] Implementar `PUT /api/categorias/{id}` reusando `ValidacionDeLaCategoria` entera.
      **El tipo no viaja en la petición**: cambiarlo movería de tipo a los movimientos que la usan
- [ ] T043 [TEST] [US2] Agregar el `PUT` a `ContratoCategoriasTests`, y declarar `CategoriaEditada`
      en `tipos.ts` y `renombrarCategoria` en `cliente.ts`
- [ ] T044 [VERIFY] [US2] Puerta del backend y del frontend completas, con su salida
...
- [ ] T045 [TEST] [US3] AC-05 en `CategoriasPropiasTests`: después de la baja, la categoría **no**
      está en `GET /api/categorias` y **sí** sigue nombrando sus movimientos en el listado
- [ ] T046 [TEST] [US3] **AC-06 y FR-011, el test que sostiene la feature**: se guarda el resumen
...
- [ ] T047 [TEST] [P] [US3] AC-09: crear una categoría con el mismo nombre y tipo que una dada de
...
- [ ] T048 [TEST] [P] [US3] La baja es idempotente: dos `DELETE` seguidos devuelven `204` los dos
- [ ] T049 [TEST] [P] [US3] AC-03 y AC-11 en el `DELETE`: `403` sobre una predefinida, `404` sobre
      una de otra cuenta, con el mismo cuerpo que un id inexistente
- [ ] T050 [US3] Implementar `DELETE /api/categorias/{id}`: apaga `Activa` y escribe
      `Discriminador = Id` **en el mismo `UPDATE`**. `204` sin cuerpo
- [ ] T051 [TEST] [US3] FR-023 en `ValidacionMovimientoTests`: editar un movimiento **sin cambiarle
...
- [ ] T052 [US3] Agregar la condición a la edición en
...
- [ ] T053 [TEST] [US3] Agregar el `DELETE` a `ContratoCategoriasTests` y `darDeBajaCategoria` a
      `cliente.ts`
- [ ] T054 [US3] Crear `backend/GestionGastos.Api.Tests/Integracion/AislamientoDeCategoriasTests.cs`
