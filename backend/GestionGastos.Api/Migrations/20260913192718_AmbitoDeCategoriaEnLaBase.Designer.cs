// <auto-generated />
using System;
using GestionGastos.Api.Persistencia;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace GestionGastos.Api.Migrations
{
    [DbContext(typeof(GestionGastosDbContext))]
    [Migration("20260913192718_AmbitoDeCategoriaEnLaBase")]
    partial class AmbitoDeCategoriaEnLaBase
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.19")
                .HasAnnotation("Relational:MaxIdentifierLength", 64);

            MySqlModelBuilderExtensions.AutoIncrementColumns(modelBuilder);

            modelBuilder.Entity("GestionGastos.Api.Dominio.Categoria", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<ulong>("Activa")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit(1)")
                        .HasDefaultValue(1ul)
                        .HasColumnName("activa");

                    b.Property<long>("Discriminador")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasDefaultValue(0L)
                        .HasColumnName("discriminador");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)")
                        .HasColumnName("nombre");

                    b.Property<sbyte>("Tipo")
                        .HasColumnType("tinyint")
                        .HasColumnName("tipo");

                    b.Property<long>("UsuarioId")
                        .HasColumnType("bigint")
                        .HasColumnName("usuario_id");

                    b.HasKey("Id");

                    b.HasAlternateKey("Id", "UsuarioId")
                        .HasName("ak_categoria_id_usuario");

                    b.HasIndex("UsuarioId", "Nombre", "Tipo", "Discriminador")
                        .IsUnique()
                        .HasDatabaseName("ux_categoria_ambito_nombre_tipo");

                    b.ToTable("categoria", (string)null);
                });

            modelBuilder.Entity("GestionGastos.Api.Dominio.IntentoDeAcceso", b =>
                {
                    b.Property<string>("Email")
                        .HasMaxLength(254)
                        .HasColumnType("varchar(254)")
                        .HasColumnName("email")
                        .UseCollation("utf8mb4_0900_ai_ci");

                    b.Property<byte>("FallosConsecutivos")
                        .HasColumnType("tinyint unsigned")
                        .HasColumnName("fallos_consecutivos");

                    b.Property<DateTime>("UltimoFallo")
                        .HasColumnType("datetime(6)")
                        .HasColumnName("ultimo_fallo");

                    b.HasKey("Email");

                    b.HasIndex("UltimoFallo")
                        .HasDatabaseName("ix_intento_de_acceso_ultimo_fallo");

                    b.ToTable("intento_de_acceso", (string)null);
                });

            modelBuilder.Entity("GestionGastos.Api.Dominio.Moneda", b =>
                {
                    b.Property<short>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("smallint")
                        .HasColumnName("id");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<short>("Id"));

                    b.Property<string>("Codigo")
                        .IsRequired()
                        .HasColumnType("char(3)")
                        .HasColumnName("codigo");

                    b.Property<byte>("Decimales")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("tinyint unsigned")
                        .HasDefaultValue((byte)2)
                        .HasColumnName("decimales");

                    b.Property<ulong>("EsPredeterminada")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit(1)")
                        .HasDefaultValue(0ul)
                        .HasColumnName("es_predeterminada");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("varchar(30)")
                        .HasColumnName("nombre");

                    b.Property<string>("Simbolo")
                        .IsRequired()
                        .HasMaxLength(5)
                        .HasColumnType("varchar(5)")
                        .HasColumnName("simbolo");

                    b.HasKey("Id");

                    b.HasIndex("Codigo")
                        .IsUnique();

                    b.ToTable("moneda", null, t =>
                        {
                            t.HasCheckConstraint("ck_moneda_codigo_tres_letras", "codigo REGEXP '^[A-Za-z]{3}$'");
                        });

                    b.HasData(
                        new
                        {
                            Id = (short)1,
                            Codigo = "ARS",
                            Decimales = (byte)2,
                            EsPredeterminada = 1ul,
                            Nombre = "Peso argentino",
                            Simbolo = "$"
                        },
                        new
                        {
                            Id = (short)2,
                            Codigo = "USD",
                            Decimales = (byte)2,
                            EsPredeterminada = 0ul,
                            Nombre = "Dólar estadounidense",
                            Simbolo = "US$"
                        });
                });

            modelBuilder.Entity("GestionGastos.Api.Dominio.Movimiento", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<long>("Id"));

                    b.Property<int>("CategoriaId")
                        .HasColumnType("int")
                        .HasColumnName("categoria_id");

                    b.Property<DateOnly>("Fecha")
                        .HasColumnType("date")
                        .HasColumnName("fecha");

                    b.Property<short>("MonedaId")
                        .HasColumnType("smallint")
                        .HasColumnName("moneda_id");

                    b.Property<decimal>("Monto")
                        .HasColumnType("decimal(11,2)")
                        .HasColumnName("monto");

                    b.Property<string>("Nota")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)")
                        .HasColumnName("nota");

                    b.Property<sbyte>("Tipo")
                        .HasColumnType("tinyint")
                        .HasColumnName("tipo");

                    b.Property<long>("UsuarioId")
                        .HasColumnType("bigint")
                        .HasColumnName("usuario_id");

                    b.HasKey("Id");

                    b.HasIndex("MonedaId");

                    b.HasIndex("CategoriaId", "UsuarioId");

                    b.HasIndex("UsuarioId", "Fecha", "Id")
                        .IsDescending(false, true, true)
                        .HasDatabaseName("ix_movimiento_usuario_fecha");

                    b.ToTable("movimiento", null, t =>
                        {
                            t.HasCheckConstraint("ck_movimiento_monto_positivo", "monto > 0");

                            t.HasCheckConstraint("ck_movimiento_nota_sin_cadena_vacia", "nota IS NULL OR nota <> ''");
                        });
                });

            modelBuilder.Entity("GestionGastos.Api.Dominio.Usuario", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<long>("Id"));

                    b.Property<string>("ContrasenaHash")
                        .IsRequired()
                        .HasMaxLength(72)
                        .HasColumnType("varchar(72)")
                        .HasColumnName("contrasena_hash");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(254)
                        .HasColumnType("varchar(254)")
                        .HasColumnName("email")
                        .UseCollation("utf8mb4_0900_ai_ci");

                    b.HasKey("Id");

                    b.HasIndex("Email")
                        .IsUnique();

                    b.ToTable("usuario", (string)null);
                });

            modelBuilder.Entity("GestionGastos.Api.Dominio.Categoria", b =>
                {
                    b.HasOne("GestionGastos.Api.Dominio.Usuario", "Usuario")
                        .WithMany()
                        .HasForeignKey("UsuarioId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("GestionGastos.Api.Dominio.Movimiento", b =>
                {
                    b.HasOne("GestionGastos.Api.Dominio.Moneda", "Moneda")
                        .WithMany()
                        .HasForeignKey("MonedaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("GestionGastos.Api.Dominio.Usuario", null)
                        .WithMany()
                        .HasForeignKey("UsuarioId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("GestionGastos.Api.Dominio.Categoria", "Categoria")
                        .WithMany()
                        .HasForeignKey("CategoriaId", "UsuarioId")
                        .HasPrincipalKey("Id", "UsuarioId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_movimiento_categoria_del_ambito");

                    b.Navigation("Categoria");

                    b.Navigation("Moneda");
                });
#pragma warning restore 612, 618
        }
    }
}
